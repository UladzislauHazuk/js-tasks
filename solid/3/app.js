// Реализуйте класс ServerGetAll. Обязательными функциями считаются функции controller, service, repository. Цепочка взаимодействия между методами следующая:
...
class ServerGetAll {
    ...
    controller() {
        try {
            ...
            document.querySelector('.result').innerHTML = JSON.stringify(service);
        } catch (error) {
            alert(error.message);
        }
    }
    ...
    service() {
        ...
        if(!check) throw new Error('Ошибка');
        return check;
    }
