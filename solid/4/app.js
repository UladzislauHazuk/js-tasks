...
// middleware -> controller -> service -> c, где:
...
class ServerById {
    ...
    controller(obj) {
        try {
            ...
            document.querySelector('.result').innerHTML = JSON.stringify(this.service(objParsed));
        } catch (error) {
            alert(error.message);
        }
    }
    ...
    service(obj) {
        ...
        if (!data.length) throw new Error('Пустой массив');
        return data;
    }
